...
public static class PilishString
{
    ...
    static ulong intPI = 314159265358979;
    ...
    public static string Evaluate(string input)
    {
        ...
        int remainingLetters = input.Length;
        ...
        for (int i = 14; i > 0; i--)
        {
            ...
            int workingLength = GetNthDigit(intPI, i);

            if (workingLength > remainingLetters)
            {
                //Console.WriteLine(input.Substring(nextWordStart, remainingLetters));
                theString += input.Substring(nextWordStart, remainingLetters);

                for (int r = 0; r < workingLength - remainingLetters; r++)
                {
                    theString += input.Substring(inputLength - 1);
                }

                remainingLetters = 0;
            } else
            {
                //Console.WriteLine(input.Substring(nextWordStart, workingLength));
                theString += input.Substring(nextWordStart, workingLength);
            }

            remainingLetters -= workingLength;
            //Console.WriteLine("remaining letters in word: " + remainingLetters);
            nextWordStart += workingLength;
            //Console.WriteLine("next starting at index: " + nextWordStart);
    ...
    /// <summary>
    /// Returns the 0-indexed nth digit of a number, indexing from the smallest unit to the largest
    /// </summary>
    /// <param name="number"></param>
    /// <param name="index"></param>
    /// <returns></returns>
    public static int GetNthDigit(ulong number, int index)
    {
        ulong value = (number / (ulong)MathF.Pow(10, index)) % 10;
        //Console.WriteLine("Length of next word: " + (int)value);
        return (int)value;
    }
}